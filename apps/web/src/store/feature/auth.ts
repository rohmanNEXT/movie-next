...
export const useAuthStore = create<AuthState>((set) => ({
  ...
  registerUser: async (formData) => {
    ...
    try {
      const { data } = await connectApi.post<ApiResponse>(
        '/register',
        formData,
      );
      set({ loading: false });
  ...
  loginUser: async (credentials) => {
    set({ loading: true, error: null });
    try {
      const { data } = await connectApi.post<
  ...
  forgotPassword: async (email) => {
    set({ loading: true, error: null });
    try {
      const { data } = await connectApi.post<ApiResponse>('/forgot-password', {
        email,
      });
      set({ loading: false });
      return data;
  ...
  resetPassword: async (token, password) => {
    set({ loading: true, error: null });
    try {
      const { data } = await connectApi.post<ApiResponse>('/reset-password', {
        token,
        password,
      });
      set({ loading: false });
  ...
  googleLogin: async (idToken, isRegister = false) => {
    ...
    try {
      const { data } = await connectApi.post<
        ApiResponse<{ token: string; user: User }>
      >('/auth/google', { idToken, isRegister });
      set({ user: data.data.user, token: data.data.token, loading: false });
      saveAuthToLocal(data.data.user, data.data.token);
      return data;
    } catch (err: any) {

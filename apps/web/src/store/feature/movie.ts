...
export const useMovieStore = create<MovieStore>((set, get) => ({
  ...
  getData: async (params) => {
    ...
    try {
      const { data } = await connectApi.get<ApiResponse<Movie[]>>('/movies', {
        params,
      });
  ...
  addMovie: async (movieData) => {
    try {
      const { data } = await connectApi.post<ApiResponse<Movie>>(
        '/movie',
        movieData,
      );
  ...
  editMovie: async (id, movieData) => {
    try {
      const { data } = await connectApi.patch<ApiResponse<Movie>>(
        `/movie/${id}`,
        movieData,
      );
      set((state) => ({
  ...
  fetchWishlist: async () => {
    try {
      ...
      if (typeof window !== 'undefined')
        localStorage.setItem('chill_myList', JSON.stringify(movieIds));
    } catch (err) {
  ...
  toggleMyList: async (movieId) => {
    ...
    const newList = exists
      ? myList.filter((id) => id !== movieId)
      : [...myList, movieId];

    // Optimistic update
    set({ myList: newList });
    if (typeof window !== 'undefined')
      localStorage.setItem('chill_myList', JSON.stringify(newList));
    ...
    // Only sync if user is actually logged in (token exists in both store and localStorage)
    if (token && localStorageToken) {
      try {
        ...
        // If 401 error, clear invalid token and revert to local storage only
        ...
        set({ myList });
        if (typeof window !== 'undefined')
          localStorage.setItem('chill_myList', JSON.stringify(myList));
      }
    }
  },
  ...
  getMovie: async (id) => {
    ...
    try {
      const { data } = await connectApi.get<
        ApiResponse<Movie & { related: Movie[] }>
      >(`/movie/${id}`);
  ...
  getTrending: async () => {
    try {
      const { data } =
        await connectApi.get<ApiResponse<Movie[]>>('/movies/trending');

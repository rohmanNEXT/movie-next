...
import { FiCheck, FiZap, FiShield, FiStar, FiLoader } from 'react-icons/fi';
...
const iconMap: Record<string, React.ReactNode> = {
  Essential: <FiShield size={22} />,
  Professional: <FiZap size={22} />,
  Ultimate: <FiStar size={22} />,
};
...
const fallbackPlans: Package[] = [
  {
    ...
    durationDays: 30,
    features:
      'Kualitas 720p (HD) | Tonton di 2 perangkat | Bebas iklan premium | Download movie offline',
    isActive: true,
  },
  {
    id: 2,
  ...
  },
  {
    ...
    durationDays: 30,
    features:
      'Kualitas 4K + HDR10 | Tonton di 6 perangkat | Dolby Atmos & Vision | Akses awal movie baru | Kualitas bit-rate tinggi',
    isActive: true,
  },
];
...
const SubscribePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPackages = async () => {
      try {
        const { data } =
          await connectApi.get<ApiResponse<Package[]>>('/packages');
        // Always use fallback plans to ensure consistent pricing
  ...
  if (mounted && user?.subscription?.isActive) {
    return (
      <div className="min-h-screen bg-brand-dark flex flex-col items-center justify-center text-white">
        ...
        <Link
          href="/"
          className="px-6 py-3 bg-purple-500 hover:bg-purple-600 rounded-full text-sm font-semibold transition-all cursor-pointer"
        >
          Kembali ke Beranda
        </Link>
      </div>
  ...
  const handleSubscribe = async (pkg: Package) => {
    ...
    // Restrict Admin and SuperAdmin from subscribing
    if (user.role === 'admin' || user.role === 'superadmin') {
      toast.error('Hanya user dengan role User yang dapat berlangganan.');
      return;
    }
    ...
    <div className="min-h-screen bg-brand-dark text-white">
      ...
      <main className="pt-52 pb-48 px-6 md:px-12 max-w-6xl mx-auto">
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-8">
          ...
          >
            Pilih paket sesuai gaya hidup digital Anda, batalkan kapan saja
        ...
        <div className="grid md:grid-cols-3 gap-6 lg:gap-8">
          {displayPackages.map((pkg, index) => {
            ...
            const icon = iconMap[pkg.name] || <FiShield size={22} />;
            ...
            return (
              ...
              >
                ...
                <div
                  className={`w-12 h-12 rounded-xl ${color} flex items-center justify-center mb-6`}
                >
                  {icon}
                </div>
                ...
                <ul className="space-y-3.5 mb-10 grow">
                  {features.map((feature, i) => (
                    <li
                      key={i}
                      className="flex items-start gap-3 text-gray-400 text-[13px] leading-snug"
                    >
                      <FiCheck
                        size={14}
        ...
        <div className="mt-32 pt-20 text-center space-y-6">
          <div className="flex items-center justify-center gap-6 opacity-30 grayscale">
            <FiShield size={24} /> <FiZap size={24} /> <FiStar size={24} />
          </div>
          <div>
            ...
            <p className="text-[9px] font-medium text-gray-600 max-w-sm mx-auto leading-relaxed">
              Transaksi diproses menggunakan saldo akun Anda. Pastikan saldo
              mencukupi.
            </p>
          </div>
        </div>

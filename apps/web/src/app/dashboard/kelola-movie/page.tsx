...
const MovieManagementComponent: React.FC = () => {
  const { movies, loading, getData, addMovie, editMovie, deleteMovie, meta } =
    useMovieStore();
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Check if superadmin is trying to add more than 6 movies
    if (!editingId && user?.role === 'superadmin' && movies.length >= 6) {
      toast.error('Maksimal hanya 6 film untuk superadmin');
      return;
    }
  ...
  const uniqueYears = Array.from(
    new Set(
      movies.map((m: Movie) => m.year).filter((y): y is number => y !== null),
    ),
            ...
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-16">
              <div className="space-y-4">
                ...
                <p className="text-gray-600 text-xs max-w-xs leading-relaxed">
                  Pusat kendali katalog movie, serial, dan anime untuk pantau
                  dan perbarui konten chill secara real-time.
                </p>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-4 items-center">
                ...
                <div className="relative group flex items-center bg-white/3 border border-white/10 rounded-full focus-within:ring-2 focus-within:ring-white/20 focus-within:bg-white/5 transition-all w-full sm:w-[380px]">
                  ...
                  <div className="pl-4 pr-2">
                    <LuSearch
                      className="text-gray-500 group-focus-within:text-purple-500 transition-colors pointer-events-none"
                      size={16}
                    />
                  </div>
                  ...
                  <div className="relative shrink-0">
                    ...
                    >
                      <span className="font-medium">
                        {searchYear === 'All' ? 'Tahun' : searchYear}
                      </span>
                      <LuChevronDown
                        size={14}
            ...
            <div className="flex flex-col">
                  ...
                  <table className="w-full text-left border-collapse table-fixed">
                    <thead>
                      <tr className="bg-white/5 border-b border-white/10">
                        <th className="px-10 py-6 text-xs font-semibold uppercase tracking-widest text-gray-500 w-16 text-center">
                          #
                        </th>
                        <th className="px-10 py-6 text-xs font-semibold uppercase tracking-widest text-gray-500">
                          Katalog
                        </th>
                        <th className="px-10 py-6 text-xs font-semibold uppercase tracking-widest text-gray-500 w-32 text-center">
                          Genre
                        </th>
                        <th className="px-10 py-6 text-xs font-semibold uppercase tracking-widest text-gray-500 w-28 text-center">
                          Rating
                        </th>
                        <th className="px-10 py-6 text-xs font-semibold uppercase tracking-widest text-gray-500 w-28 text-center">
                          Tahun
                        </th>
                        <th className="px-10 py-6 text-xs font-semibold uppercase tracking-widest text-gray-500 text-right w-40">
                          Tindakan
                        </th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-white/5">
                      {loading && movies.length === 0 ? (
                        <tr>
                          <td colSpan={6} className="px-10 py-56">
                            <div className="flex flex-col items-center gap-8">
                              ...
                              <span className="text-gray-500 font-semibold uppercase tracking-[0.3em] text-[10px]">
                        ...
                        <tr>
                          <td
                            colSpan={6}
                            className="px-10 py-40 text-center text-gray-500 font-medium"
                          >
                            ...
                            >
                              ...
                              <td className="px-10 py-6">
                                <div className="flex items-center gap-6">
                                  ...
                                  <div>
                                    <div className="font-semibold text-base text-white/80 group-hover:text-white transition-colors line-clamp-1">
                                      {movie.title}
                                    </div>
              ...
              >
                {loading && movies.length === 0 ? (
                  ...
                  <div className="col-span-full py-40 text-center text-gray-500">
                    Tidak ada data.
                  </div>
                ) : (
                  <>
                    {currentMovies.map((movie: Movie) => (
                      <div
                        key={movie.id}
                        className="bg-white/3 p-8 rounded-[3rem] border border-white/5 space-y-8 hover:bg-white/5 transition-all group"
                      >
                        <div className="flex gap-8">
                          ...
                          <div className="flex-1 flex flex-col justify-center gap-2">
                            <h3 className="font-semibold text-xl text-white/80 leading-tight group-hover:text-white transition-colors line-clamp-2">
                              {movie.title}
                            </h3>
                            <div className="flex flex-wrap items-center gap-2 mt-1">
                              <span className="px-2.5 py-1 bg-white/5 rounded-lg text-[9px] font-semibold uppercase tracking-widest text-gray-400 border border-white/5">
                                {movie.category}
                              </span>
                              {movie.isNewEpisode && (
                                <span className="px-2.5 py-1 bg-white/10 text-gray-400 text-[9px] font-semibold uppercase rounded-lg border border-white/10">
                                  New
                                </span>
                              )}
                            </div>
                        ...
                        <div className="flex gap-4 p-2 bg-white/2 rounded-4xl border border-white/5">
                          <button
                            onClick={() => handleOpenModal(movie)}
                            className="flex-1 py-4 bg-white/5 hover:bg-white/10 rounded-3xl flex items-center justify-center gap-2 font-semibold text-xs text-blue-400 transition-all active:scale-95 cursor-pointer"
                          >
                            <LuPencil size={16} /> Edit
                          </button>
                          <button
                            onClick={() => handleDelete(movie.id)}
                            className="flex-1 py-4 bg-white/5 hover:bg-white/10 rounded-3xl flex items-center justify-center gap-2 font-semibold text-xs text-red-400 transition-all active:scale-95 cursor-pointer"
                          >
          ...
          <div className="fixed inset-0 z-100 flex items-center justify-center p-4">
            ...
            >
              <div className="px-10 py-10 border-b border-white/5 flex items-center justify-between bg-white/1">
                <div>
                  <h2 className="text-base sm:text-lg font-semibold tracking-tight text-gray-500 text-left">
              ...
              <form onSubmit={handleSave}>
                <div className="p-8 md:p-12 space-y-8 max-h-[65vh] overflow-y-auto custom-scrollbar pr-4 md:pr-6 text-left">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <div className="space-y-3">
                      <label className="block text-[10px] font-semibold text-gray-400/80 uppercase tracking-widest ml-1 mb-1">
                        Nama Judul
                      </label>
                      <input
                        type="text"
                    ...
                    <div className="space-y-3">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setFormData({ ...formData, imdbLink: e.target.value })
                        }
                      />
                    </div>
                    <div className="space-y-3">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 p-6 bg-white/2 rounded-4xl border border-white/5">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <label
                          htmlFor="isNew"
                          className="text-sm font-semibold text-white/90 cursor-pointer block"
                        >
                          Tandai sebagai Episode Baru
                        </label>
                        <span className="text-[10px] text-gray-500 font-semibold uppercase tracking-wider">
                          Akan muncul label khusus di katalog
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="p-8 border-t border-white/5 flex flex-col sm:flex-row justify-end gap-4 bg-white/1">
                  ...
                  >
                    <LuSave size={16} />{' '}
                    {editingId ? 'Simpan Konten' : 'Tayangkan Konten'}
                  </button>
                </div>
              </form>
            </motion.div>

...
const SaldoPage: React.FC = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const { data } = await connectApi.get(
          `/payment/transactions?page=${page}&limit=6`,
        );
  ...
  const handleWithdraw = async (e: React.FormEvent) => {
    ...
    if (amount < 10000) {
      toast.error('Minimum penarikan adalah Rp 10.000');
      return;
    }

    if (amount > 10000000) {
      toast.error('Maksimum penarikan adalah Rp 10.000.000');
      return;
    }
    ...
    try {
      const { data } = await connectApi.post('/payment/withdraw', { amount });
      if (data.success) {
      ...
      toast.error(err.response?.data?.message || 'Gagal memproses penarikan');
    } finally {
        ...
        {loading ? (
          ...
          <div style={{ minHeight: '600px' }}>
            ...
            >
              {/* Header */}
              <div className="mb-12">
                <h1 className="text-2xl font-semibold tracking-tight text-gray-400 mb-2">
                ...
                <p className="text-gray-600 text-xs">
                  {user?.role === 'admin' || user?.role === 'superadmin'
                    ? 'Kelola saldo Anda.'
                    : 'Kelola saldo Anda dan pantau komisi referral.'}
                </p>
              </div>
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-12">
                {/* Balance Card */}
                <div className="p-6 rounded-3xl bg-white/3 border border-white/5 space-y-5 backdrop-blur-xl">
                  <div className="flex items-center gap-3">
                    <div className="w-11 h-11 rounded-2xl bg-purple-600/20 flex items-center justify-center text-purple-400">
                      <LuWallet size={22} />
                    </div>
                    <div>
                  ...
                  >
                    <div className="space-y-2">
                      ...
                      <input
                        ...
                        onChange={(e) => {
                          ...
                          setTopupAmount(
                            clean
                              ? parseInt(clean, 10).toLocaleString('id-ID')
                              : '',
                          );
                        }}
                    ...
                    {amountVal >= 10000 && (
                      <div className="p-4 bg-white/2 rounded-2xl border border-white/5 space-y-2 text-xs">
                        ...
                        <div className="border-t border-white/5 pt-2 flex justify-between font-semibold text-white">
                          ...
                          <span>
                            Rp {totalPaymentVal.toLocaleString('id-ID')}
                          </span>
                        </div>
                      </div>
                    )}
                    ...
                    <button
                      ...
                      onClick={() => {
                        const amount = prompt(
                          'Masukkan jumlah penarikan (Min. Rp 10.000):',
                        );
                        if (amount) {
                          const cleanAmount = amount.replace(/[^\d]/g, '');
                          setWithdrawAmount(
                            cleanAmount
                              ? parseInt(cleanAmount, 10).toLocaleString(
                                  'id-ID',
                                )
                              : '',
                          );
                          handleWithdraw(new Event('submit') as any);
                        }
                      }}
                      disabled={isWithdrawing}
                      className="w-full px-5 py-3 bg-white/5 hover:bg-white/10 text-white rounded-2xl text-sm font-semibold transition-all flex items-center justify-center gap-2 cursor-pointer border border-white/5 disabled:opacity-50"
                    >
                      {isWithdrawing ? (
                        <LuLoader size={16} className="animate-spin" />
                      ) : (
                ...
                {/* Referral Card */}
                {user?.role !== 'admin' && user?.role !== 'superadmin' && (
                  <div className="p-6 rounded-3xl bg-white/3 border border-white/5 space-y-5 backdrop-blur-xl flex flex-col justify-between">
                    <div>
                      <div className="flex items-center gap-3 mb-5">
                        <div className="w-11 h-11 rounded-2xl bg-yellow-500/20 flex items-center justify-center text-yellow-400">
                          <LuWallet size={22} />
                        </div>
                        <div>
                          <p className="text-[10px] font-semibold uppercase tracking-widest text-white/30">
                          ...
                          <p className="text-2xl font-semibold text-white">
                            Rp {totalReferralCommission.toLocaleString('id-ID')}
                          </p>
                        </div>
                      </div>
                    ...
                    <p className="text-xs text-white/30 leading-relaxed mt-4">
                      Bagikan kode referral Anda ke teman. Dapatkan komisi 10%
                      setiap kali mereka melakukan transaksi valid.
                    </p>
                  </div>
                )}
              </div>
                  ...
                  <table className="min-w-[820px] w-full text-left border-collapse table-fixed">
                    <thead>
                      <tr className="bg-white/5 w-full border-b border-white/5">
                        <th className="w-[100px] px-6 py-2 text-xs font-semibold uppercase tracking-widest text-gray-500">
                          Tanggal
                        </th>
                        <th className="w-[420px] px-6 py-2 text-xs font-semibold uppercase tracking-widest text-gray-500">
                          Deskripsi
                        </th>
                        <th className="w-[200px] px-6 py-2 text-xs font-semibold uppercase tracking-widest text-gray-500">
                          Tipe
                        </th>
                        <th className="w-[120px] px-6 py-2 text-xs font-semibold uppercase tracking-widest text-gray-500">
                          Jumlah
                        </th>
                        <th className="w-[120px] px-6 py-2 text-xs font-semibold uppercase tracking-widest text-gray-500">
                          Status
                    ...
                    <tbody className="divide-y divide-white/5">
                      {transactions.length === 0 ? (
                        <tr>
                          <td
                            colSpan={5}
                            className="px-6 py-10 text-center text-gray-500 font-medium text-xs"
                          >
                            Belum ada transaksi.
                          </td>
                        </tr>
                        ...
                        transactions.map((tx) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <td
                                className="px-6 text-xs font-medium text-white/80 truncate"
                              ...
                              <td
                                className={`px-6 py-2 font-semibold text-xs truncate ${isIncome ? 'text-green-400' : 'text-red-400'}`}
                              >
                                {isIncome ? '+' : '-'} Rp{' '}

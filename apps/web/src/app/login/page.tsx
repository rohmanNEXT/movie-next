...
import { LuPlay, LuEye, LuEyeOff } from 'react-icons/lu';
...
export default function LoginPage() {
  ...
  const handleGoogleOAuth = async () => {
    const clientId =
      clientIds.google || process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || '';
            ...
            googleLogin(idToken, false)
              .then((result: any) => {
                if (result?.success || !result?.error) {
                  ...
                  router.push(
                    ['admin', 'superadmin'].includes(result?.data?.user?.role)
                      ? '/dashboard'
                      : '/',
                  );
                }
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values: any) => {
      try {
        ...
        if (response.success) {
          toast.success(
            response.message || `Halo ${response.data.user.username}!`,
          );
    ...
    <div className="min-h-screen bg-brand-dark flex items-center justify-center p-4 sm:p-6 relative overflow-hidden">
      ...
      >
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6">
          ...
          {/* Forgot */}
          <div className="cursor-pointer text-right text-xs text-gray-400 -mt-2">
          ...
          <button
            type="submit"
            className="w-full cursor-pointer py-2 md:py-2.5 bg-purple-600 hover:bg-purple-700 text-white font-semibold rounded-full transition active:scale-[0.98] text-xs md:text-sm shadow-lg shadow-purple-600/20"
          >
            Masuk
          </button>
          ...
          <div className="space-y-3">
            <button
              ...
              className="w-full cursor-pointer py-2 md:py-2.5 glass hover:bg-white/10 text-white font-semibold rounded-full transition flex items-center justify-center gap-3 active:scale-[0.98] text-xs md:text-sm"
            >
              <Image

...
export class MovieController {
  ...
  async getTrending(req: Request, res: Response) {
    try {
      ...
      const movieIds = viewCounts.map((v: any) => v.movieId);
      ...
      const viewCountMap = new Map<number, number>(viewCounts.map((v: any) => [v.movieId, v._count.movieId]));
      const sortedMovies = movies
        .map((m: any) => ({ ...m, viewCount: viewCountMap.get(m.id) || 0 }))
        .sort((a: any, b: any) => Number(b.viewCount) - Number(a.viewCount));
